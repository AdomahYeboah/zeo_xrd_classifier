"""
comparison-based global calibration (class-blind outputs only).

comparison: per-class comparison of SELECT idealized IZA references (FAU/FER/LTA/MFI .xy)
    against class training patterns; governing values pooled by MEDIAN over the 4 comparisons.
measured: Oviedo-style statistics over the 7 training patterns (FWHM quantiles, peak shifts
    vs IZA tops, exp/sim height ratios, residual noise). No per-class outputs used
    downstream: both dicts are single sets with no framework branching.

Usage:
    python calibrate_comparison_globals.py --base <project_exp> --out-dir <calibration_out>
"""
...
def main():
    ap = argparse.ArgumentParser(description="COMP/POOL class-blind global calibration")
    ...
    for fw in TARGET_FRAMEWORKS:
        print(f"  training {fw}: {len(by_fw[fw])}")
    ...
    comp_broad = [round(clamp(0.6 * med_fw, 0.30, 3.00), 2),
                round(clamp(1.5 * med_fw, 0.30, 3.00), 2)]
    comp_eta = [0.30, 0.80] if float(np.mean(
    ...
    for fw in TARGET_FRAMEWORKS:
        for b in per_class[fw]["bg"]:
            ...
            if b["max"] > 0.15:
                bg_hits += 1
                bg_pos.append(b["at"])
    n_train = sum(per_class[fw]["n_exp"] for fw in TARGET_FRAMEWORKS)
    q25, q75 = (round(float(np.percentile(bg_pos, 25)), 1), round(float(np.percentile(bg_pos, 75)), 1)) if bg_pos else (20.0, 20.0)
    COMP = {
        "name": "comparison-calibrated set (median over 4 IZA-vs-training comparisons)",
        "broadening": {"fwhm_min": comp_broad[0], "fwhm_max": comp_broad[1],
                       "eta_min": comp_eta[0], "eta_max": comp_eta[1]},
        ...
        "amorphous": {"center_min": q25,
                      ...
                      "apply_prob": round(bg_hits / max(1, n_train), 2)},
        "low_angle_hump": None,
    ...
    POOL = {
        "name": "pooled measured set (training-pattern quantiles)",
    ...
    stats = {
        "n_train_fwhm": len(all_fwhm),
        "train_fwhm_p10_p50_p90": [round(f10, 3), round(float(np.median(all_fwhm)), 3), round(f90, 3)],
    ...
    with open(os.path.join(out_dir, "globals_comparison.json"), "w") as f:
        json.dump({"comparison": COMP, "measured": POOL, "stats": stats,
                   "provenance": "medians over 4 IZA-vs-training comparisons; "
                   "quantiles over pooled training-pattern measurements; orientation identity, "
                   "strain 0.002, comparison widths, pooled eta are uncalibrated carryovers"},
                  f, indent=2)
    print("\ncomparison broadening:", COMP["broadening"], "eta:", COMP["broadening"]["eta_min"], COMP["broadening"]["eta_max"])
    print("comparison amorphous:", {k: COMP["amorphous"][k] for k in ("center_min", "amp_min", "amp_max", "apply_prob")})
    print("pooled broadening:", POOL["broadening"])
    print("pooled rescale:", POOL["rescale"], "shift_max:", POOL["shift_max"], "noise:", POOL["noise_scale"])
    print("stats:", json.dumps({k: v for k, v in stats.items() if k != "per_class"}, indent=2))
    print(f"\nSaved globals_comparison.json to {out_dir}")
